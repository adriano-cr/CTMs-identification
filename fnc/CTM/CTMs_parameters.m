...
try
    ...
    flow_in_clean = filloutliers(flow_in,"nearest","percentiles",[45 99]);
    flow_out_clean = filloutliers(flow_out,"clip","percentiles",[30 99]);

    x_flow = (linspace(0,24,length(flow_out_clean)))';
    [f_in_poly,gof] = fit(x_flow,flow_in_clean','poly7','Normalize','on','Robust','Bisquare');

    [f_in_fou,gof] = fit(x_flow,flow_in_clean','fourier5','Normalize','on','Robust','Bisquare');

    [f_out_poly,gof] = fit(x_flow,flow_out_clean','poly7','Normalize','on','Robust','Bisquare');

    [f_out_fou,gof] = fit(x_flow,flow_out_clean','fourier5','Normalize','on','Robust','Bisquare');
    ...
    f_beta = fit(x_beta',beta','poly2', 'Exclude', beta_outliers);
    ...
    for i=1:window:length(io_poly)-window+1
        input=0;
        output=0;
        for j=i:i+window-1
            if(round(io_poly(j))>0)
                input = input + round(io_poly(j));
            else
                output = output + round(io_poly(j));
            end
        end
        ...
        for j=i:i+window-1
            if(round(io_fou(j))>0)
                input = input + round(io_fou(j));
            else
                output = output + round(io_fou(j));
            end
        end
    ...
    for k=35:85
        ...
        syms f_in_poly_sym(p1,p2,p3,p4,p5,p6,p7,p8,x)
        f_in_poly_sym(p1,p2,p3,p4,p5,p6,p7,p8,x)=p1*x^7+p2*x^6+p3*x^5+p4*x^4+p5*x^3+p6*x^2+p7*x+p8;
        f_in_poly_sym(x) = subs(f_in_poly_sym(p1,p2,p3,p4,p5,p6,p7,p8,x), {p1, p2, p3, p4, p5, p6, p7, p8}, coefficientValues_f_in_poly);

        coefficientValues_f_out_poly = coeffvalues(f_out_poly);
        syms f_out_poly_sym(p1,p2,p3,p4,p5,p6,p7,p8,x)
        f_out_poly_sym(p1,p2,p3,p4,p5,p6,p7,p8,x)=p1*x^7+p2*x^6+p3*x^5+p4*x^4+p5*x^3+p6*x^2+p7*x+p8;
        f_out_poly_sym(x) = subs(f_out_poly_sym(p1,p2,p3,p4,p5,p6,p7,p8,x), {p1, p2, p3, p4, p5, p6, p7, p8}, coefficientValues_f_out_poly);
    ...
    if(opt.display>0)
        ...
        hold on
        plot(fit(delay_fou_plot(:,1),delay_fou_plot(:,2),'poly1'))
        %ylim([0 300])
        grid on
        ylabel("min")
        ...
        hold on
        plot(fit(delay_poly_plot(:,1),delay_poly_plot(:,2),'poly1'))
        %ylim([0 300])
        ylabel("min")
        grid on
