    ...
    if opt.load 
        ...
        if ~isempty(min_freq) && sensor(1).sample_time(1) > 1/min_freq
            ...
            for k = 1: length(sensors_id)
                ...
                % We must divide by the frequency in order to obtain how
                ...
                % minutes hencewe have to multiply 1/60 to achieve  
        ...
        for j = 1:length(sensors_id)
            % flow = vehicle_num*(sample_time/1h)
            flow = sensor(j).vehicle_number./sensor(j).sample_time;%.*60; % [veh/h]
            % density = flow*speed
